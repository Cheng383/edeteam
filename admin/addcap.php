			...
			<div class="row">
				<div class="col-md-9 panel-upload">
					<div class="breadcrumb">
						<h3>Agregar un Capitulo</h3>
					</div>
					<form action="../form/insertcap.php" enctype="multipart/form-data" method="post">
						<div class="form-group">
							<input type="text" name="nombre" placeholder="Nombre del capitulo" class="form-control">
							<p>Ejemplo: (Nombre del comic capitulo #Orden)</p>
						</div>
						<div class="form-group">
						...
						try {
							include '../config/conexion.php';
							$sql = "SELECT comic.Nombre,comic.Id_c FROM usuarios INNER JOIN  comic ON usuarios.Id =  comic.Id_autor WHERE  Id = '" . $crow['Id'] . "' ";
							$lista_cap = $base->query($sql);
							echo "<select name='opciones' class='form-control'>";
							while ($rowCap=$lista_cap->fetch(PDO::FETCH_ASSOC)) {
								echo "<option value='". $rowCap['Id_c'] ."'>". $rowCap['Nombre'] ."";
							}
							echo "</select>";							
						} catch (Exception $e) {
					...
					?>
						...
						<div class="form-group">
							<input type="text" name="url" class="form-control" placeholder="Escribe nuevamente el nombre del capitulo con su orden">
							<p>Ejemplo: (Nombre del comic capitulo #Orden)</p>
						</div>
						<div class="form-group">
