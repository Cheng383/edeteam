...
?>
	<header>
		...
		<script type="text/javascript">
			function seguro() {
				// body...
				recordar = confirm("¿Esta seguro que deseas eliminar este comic?");
				return recordar;
			}
		</script>
				...
				<div class="col-md-9 panel-upload">
					...
					<div class="panel panel-defaul">
						...
						<table class="table">
							...
							include '../config/conexion.php';
							$sql = "SELECT * FROM usuarios WHERE Email = '" . $_SESSION['usuario'] . "'";
							$listacap = $base->query($sql);

							if($crow=$listacap->fetch(PDO::FETCH_ASSOC)){
								$lista_cap = $base->query("SELECT * FROM usuarios INNER JOIN  comic ON usuarios.Id =  comic.Id_autor WHERE  Id = '" . $crow['Id'] . "' ")->fetchAll(PDO::FETCH_OBJ);;
							...
							echo "<tr>
								...
								<td>". $cap->Generos . "</td>
								<td>". $cap->Estado . "</td>
								<td><a onclick='seguro()' href='../form/quitar.php?Id_c=". $cap->Id_c ."'><span class='glyphicon glyphicon-eye-open'></span></a></a></td>
							</tr> ";
							} }?>
						</table>
					</div>
				...
				<aside class="col-md-3 sidebar-upload pull-right hidden-sm hidden-xs">
					...
					<div class="sidebar-post">
						<div class="list-group">
							...
							<a href="upload" class="list-group-item">Subir Comic</a>
							<a href="mi-lista" class="list-group-item">Lista de Comics</a>
							<a href="eliminar" class="list-group-item active">Eliminar Comics</a>
						</div>
					</div>
				</aside>
			</div>
